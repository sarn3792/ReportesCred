...
namespace Reportes
{
    public class CNBVRelavates : Report
    {
        public override String CreateFile(DataTable data)
        {
            String fileName = FileSettings.fileName("CNBVRelevantes");
            ...
            using (StreamWriter file = new StreamWriter(System.Web.HttpContext.Current.Server.MapPath(FileSettings.filePath + fileName), false))
            {
                ...
                {
                    CNBVRelevantesBean information = new CNBVRelevantesBean(); //the class validate the fields and set its correct format

                    information.PeriodoReporte = DateTime.Now.ToString("yyyyMM");
                    ...
                    //information.FechaOperacion = DateTime.Now.ToString("yyyyMMdd");
